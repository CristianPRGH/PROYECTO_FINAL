...
class Page extends Basemodel{
    ...
    protected function InsertPage($pageid, $bookid, $userid, $pageContent)
    {
        ...
        $query = "UPDATE $this->table SET Content = ? WHERE UIPage = ? AND UIBook = ? AND UIUser = ?";
        ...
        $query = "SELECT COUNT(*) as SelectedPage FROM $this->table WHERE UIPage = ? AND UIBook = ? AND UIUser = ?";
        ...
        if ($resultSelect["data"]["SelectedPage"] == 0)
        {
            ...
            $query = "INSERT INTO $this->table (UIPage, UIBook, UIUser, Content, PageNumber) VALUES (UUID(),?,?,?)";
            return parent::InsertRows($query, [$bookid, $userid, $pageContent, $pageid]);
        }
    }
